        ...
        while(1):
            ...
            for each_use in memory_uses:
                if each_use.operation == MediumLevelILOperation.MLIL_SET_VAR_SSA:
                    ...
                    # was previously assigned to ecx
                    if str(
                            read_ins.src) == 'ecx' or 'ecx' in str(
                            vars_read.var):
                ...
                if each_use.operation == MediumLevelILOperation.MLIL_STORE_SSA:
                    ...
                    for sinks in sink3:
                        if vars_written == sinks:
                            print(
                                "[*] Found a write via @ %s vars_written:%s sinks:%s a \
...
def scan_binary(bin):
    ...
    for each_func in CFG_funcs:
        ...
        if each_func.stack_adjustment.value == 8:
            retn_func_count += 1
            found = [0]
            # check the function for gadgets
            func_gadget_find(each_func, found)
            for each_callee in each_func.callees:
                found[0] = 0
                # check each callee for gadgets
                func_gadget_find(each_callee, found)
                if found[0] == 1:
                    print(
                        "[*] Function %s @ %s has a callee %s @ %s which seems useful" %
                        (each_func.symbol.full_name, hex(
                            each_func.start), each_callee.symbol.full_name, hex(
                            each_callee.start)))
...
binary_list = [
    'C:\\ProgramData\\Microsoft\\Windows Defender\\platform\\4.18.1911.3-0\\X86\\MpOav.dll',
    ...
    'C:\\Windows\\SysWOW64\\directmanipulation.dll',
    'C:\\Windows\\SysWOW64\\ninput.dll',
    'C:\\Windows\\SysWOW64\\IEUI.dll',
    'C:\\Windows\\SysWOW64\\MSHTML.dll',
    'C:\\Windows\\SysWOW64\\IDStore.dll',
    ...
    'C:\\Windows\\SysWOW64\\vm3dum_10.dll',
